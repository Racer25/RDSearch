...
var noUpdateRareDiseaseWorkflow = function()
{
    ...
    RareDiseaseDao.getRareDiseases(
        ...
        function(results)
        {
            ...
            {
                return currentValue.orphanetID >= "739";
            });*/
    ...
    NCBIAPIModule.callNCBIAPI(
        ...
        [
            ...
            function(idlist, orphanetID)
            {
                ...
                disease.init(rareDisease.orphanetID, rareDisease.name, idlist.length);
                RareDiseaseDao.addOrModifyRareDisease(disease,
                    function()
                    {
                        if(idlist !== undefined && orphanetID !== undefined && idlist.length > 0)
                        {
                            useIdsFromESearch(idlist, rareDisease, symptoms, callback);
                        }
                        else
                        {
                            console.log("No ids found for disease "+rareDisease.orphanetID +": "+rareDisease.name);
                            console.log("Trying without quote for disease "+rareDisease.orphanetID +": "+rareDisease.name);

                            let preprocessedString=rareDisease.name;
                            NCBIAPIModule.callNCBIAPI(
                                NCBIAPIModule.getIdsFromSearch,
                                [
                                    preprocessedString,
...
let useIdsFromESearch = function(idlist, rareDisease, symptoms, callback)
{
    //FILTER of 1000 publications////
    idlist = idlist.filter(function(currentValue, index, arr){return index < 1000});
    /////////////////////////////////
...
var getInformationFromJSON = function(diseases, textualinformations)
{
    ...
    for(var m =0; m<disorderList.length; m++)
    {
        ...
        if(isADisorder)
        {
            ...
            disease.init(orphanetID, name, 0);
